...
- [x] T033 [P] `src/logic/` の全公開関数に JSDoc コメント（パラメータ・返り値・例外なし説明）を追加する
- [x] T034 CI でカバレッジレポートを確認し、ラインカバレッジ 90% 以上であることを検証する（SC-001）
- [x] T035 `specs/001-nxn-cube-core-logic/quickstart.md` の全手順を実行して期待通りの出力が得られることを確認する
- [x] T036 [P] `pnpm tsc --noEmit` でコンパイルエラーがゼロであることを確認する（SC-004）
