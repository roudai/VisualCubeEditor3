...
変更された原則:
  I. 目的継承の原則 — V2 が実際にサポートする出力フォーマット（PNG 静止画と SVG）に
     合わせて、誤って含まれていた「アニメーション GIF/WebP」要件を削除。
     コアミッション記述および原則V の根拠からも「アニメーション」を削除。
     PATCH バージョン: 事実誤認の訂正であり、原則の意図に変更なし。
...
更新が必要なテンプレート:
  ✅ .specify/templates/plan-template.md — 影響なし（記述変更のみ）
  ✅ .specify/templates/tasks-template.md — 影響なし

解消された TODO:
  - srVisualizer のパッケージ名・バージョン制約: V2 が `sr-visualizer@1.0.13` を
    使用していることを確認し、技術スタック表に確定値として記載。
...
**「誰でも最小限の手順で高品質なキューブ画像（PNG / SVG）を生成できる」**
を必ず継承しなければならない。すべての設計判断はこの目標に照らして評価すること。

- V3 は V2 がサポートする出力フォーマット（静止画 PNG、埋め込み可能 SVG）をサポートしなければならない（MUST）。
...
| キューブ描画ライブラリ | sr-visualizer (npm) | ^1.0.13（V2 で使用しているバージョン） |
| 状態管理 | Pinia | ≥ 2.x |
| ビルドツール | Vite | ≥ 6.x |
| テストランナー | Vitest | ≥ 3.x |
...
**バージョン**: 1.2.1 | **制定日**: 2026-04-26 | **最終改訂**: 2026-05-01
